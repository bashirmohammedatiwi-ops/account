 ...
 * «حركة مواد — مشتريات»). Kind = 3 is mostly inventory adjustment.
 *
 * Usage:
...
async function main(options = {}) {
  ...
  const consumerProducts = incremental ? [] : await fetchConsumerPrices();
  ...
  console.log(
    `✓ ${incremental ? 'تحديث' : 'مزامنة كاملة'}: ${summary.productsUpserted} منتج، ${summary.movementsUpserted} حركة (${summary.bills} فاتورة)`,
  );
  console.log(`@SYNC_RESULT|${JSON.stringify(summary)}`);
  return summary;
}
...
module.exports = { main, fetchPurchaseMovements, fetchConsumerPrices, loadSyncState, saveSyncState };
