...
| الحساب | المستخدم | كلمة المرور | الصلاحيات |
|--------|----------|-------------|-----------|
| موظف تجهيز | `allemp` | من `EMP_PASS` في `.env` (افتراضي `000000`) | تعديل/حذف قبل تأكيد التجهيز |
| مدير | `مدير` | من `EMP_MANAGER_PASS` في `.env` (افتراضي `000000`) | تعديل/حذف حتى بعد تأكيد التجهيز |

يمكن تغيير اسم مستخدم المدير عبر `EMP_MANAGER_USER` في `.env`.
